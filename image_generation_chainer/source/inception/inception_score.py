...
def inception_forward(model, ims, batch_size):
    ...
    # # grigoris: trained on imagenet, so 1k classes(?)
    ys = ys[:, 1:1001] #0 and 1001-1008 are the dummies
    return ys


def inception_score(model, ims, batch_size=100, splits=10, verbose=True):
    ...
    xp = model.xp
    if c == 1:
        ims = xp.repeat(ims, 3, axis=1)

    if verbose:
        print('Inception: batch_size: {}, n_ims: {}, n_batches: {}'.format(batch_size, n, n_batches))
    ...
    for i in range(splits):
        # get n//split representations (from inception features).
        part = ys[(i * n // splits):((i + 1) * n // splits), :]
        # # grigoris, compute the KL: KL(p(y|x)||p(y)) where y is the classes and x the image.
        kl = part * (xp.log(part) -
...
def inception_accuracy(model, ims, labels, batch_size=100, splits=10, lab_range=None):
    """Compute the inception score for given images.
